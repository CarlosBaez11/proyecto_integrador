...
st.set_page_config(
    page_title="Ticket Classifier (EN/DE)",
    page_icon="🎟️",
    layout="centered"
)

# --- 2. Carga de Modelos con Cache ---
@st.cache_resource
def load_models():
    """Carga todos los modelos y artefactos necesarios y los almacena en caché."""
    print("Cargando modelos... (Esto solo debería aparecer una vez por sesión)")
    try:
        # A. Cargar modelos de prioridad
        priority_model = joblib.load("Priority_model.pkl")  # Asegúrate de tener este archivo
        label_encoder = joblib.load("le.pkl")               # Asegúrate de tener este archivo
        sentence_transformer = SentenceTransformer("paraphrase-multilingual-mpnet-base-v2")

        # B. Cargar modelo BERTopic
        topic_modeler = TopicModeler(model_path='bertopic_model', pickle_path='bertopic_model.pkl')
        topic_modeler.load_model(use_pickle=True)
...
st.title("🎟️ Intelligent Ticket Classifier (EN/DE)")
st.write("This tool understands English and German. Enter the ticket description to classify it. Topic and Sub-Topic will be predicted using a custom BERTopic model.")
st.markdown("---")

description = st.text_area("Enter the ticket description here:", height=150, placeholder="e.g., My computer won't turn on. / Mein Computer startet nicht.")
...
if st.button("Classify Ticket"):
    if not models:
        st.error("Models are not available. Check the console for errors.")
    elif description:
        with st.spinner("Analyzing ticket..."):

            # --- Predecir Prioridad ---
            embedding = models['sentence_transformer'].encode([description])
            priority_numeric = models['priority_model'].predict(embedding)
            predicted_priority = models['label_encoder'].inverse_transform(priority_numeric)[0]

            # --- Predecir Tópico y Subtópico con BERTopic ---
            topic_modeler = models["topic_modeler"]
            macro_topic, subtopics = topic_modeler.predict([description])

            subtopic_label = subtopics[0] if subtopics else "N/A"

            # --- Mostrar Resultados ---
            st.subheader("Classification Results:")
            col1, col2 = st.columns(2)
            with col1:
                st.info(f"**Topic (Macro):**\n\n### {macro_topic}")
            with col2:
                st.info(f"**Subtopic (Label):**\n\n### {subtopic_label}")
            st.success(f"**Suggested Priority:**\n\n# {predicted_priority.upper()}")
